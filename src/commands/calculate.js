...
.setDescription('number1').setRequired(true))
  .addStringOption(option =>
      option.setName('operator')
.setDescription('calc type').setRequired(true)
  .addChoices(
					{ name: 'plus', value: '+' },
					{ name: 'minus', value: '-' },
...
.setDescription('number2').setRequired(true)),
  async execute(interaction, client) {
    ...
    switch (operator) {
      ...
      case '/':
        ...
        break;
      default:
        return interaction.editReply('Invalid operator. Please use one of: +, -, *, /');
    }

const embed = new EmbedBuilder()
        .setColor(embeds.color)
        .setTitle(`**Calculator Of Newton**`)
        .setDescription(`**The result of ${number1} ${operatorSymbol} ${number2} = ${result}**`)
